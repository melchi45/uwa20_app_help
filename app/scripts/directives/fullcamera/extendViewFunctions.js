    ...
    $translate,SequenceModel){
        ...
        var presetAddCallBack = function(values) {
          if (values.action === 'add') {
            ...
            ModalManagerService.open('message', { 'buttonCount': 2, 
                                                  'message': "해당 프리셋이 이미 저장되어 있습니다. 덮어쓰겠습니까?" },
                                                  function() { PTZContorlService.ptzSetting(values, presetRefresh); }
                                    );
          }
        };
        ...
        scope.presetListCallback = function(result) {
          if (result.PTZPresets === undefined) {
            ...
            var list = [];
            var presetFunc = function(value) { PTZContorlService.execute(value); };
            ...
            var homeAction = function() { PTZContorlService.ptzSetting({'action' : 'home'}); };
            var addAction = function() { ModalManagerService.open('presetadd', { 'list' : list, 'buttonCount' : 2 }, presetAddCallBack); };
        ...
        scope.groupListCallback = function(result) {
          ...
          for(var index = 1 ; index <= maxGroup; index++ ) {
            list.push({'name':groupPrefix+' ' + index,
                      'action': groupFunc,
                      'value':index,
            });
          }
        ...
        scope.tourListCallback = function(result) {
          ...
          for(var index = 1 ; index <= maxTour; index++ ) {
            list.push({'name':tourPrefix +' ' + index,
                      'action': tourFunc,
                      'value':index,
            });
          }
        ...
        scope.traceListCallback = function(result) {        
          ...
          for(var index = 1 ; index <= maxTrace; index++ ) {
            list.push({'name': tracePrefix+' ' + index,
                        'action': traceFunc,
                        'value':index,
            });
          }
