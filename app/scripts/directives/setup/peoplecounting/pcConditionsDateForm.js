...
kindFramework.directive(
  'pcConditionsDateForm',
  function(
    PcConditionsDateFormModel,
    pcModalService,
    pcSetupService) {
    "use strict";
    return {
      ...
      link: function(scope, element) {
        ...
        function defaultDate(type) {
          ...
          if (type === 'from') {
            ...
            dateObj.setHours(MAX_HOUR);
            dateObj.setMinutes(MAX_MINUTE);
            dateObj.setSeconds(MAX_SECOND);
          }
        ...
        scope.pcConditionsDateForm = {
              ...
              switch (date.secondSelect) {
                ...
                case secondSubSearchOption[2].value:
                  ...
                  if (firstCalenderData.getFullYear() === secondCalenderData.getFullYear()) {
                    ...
                    //2014, 2014
                    if (
                      secondCalenderData.getFullYear() === minDate.getFullYear()
                    ) {
                      firstCalenderData.setDate(1);
                      firstCalenderData.setMonth(cameraLocalTime.getDateObj().getMonth());
          ...
          onSelectFirstCalender: function() {
            ...
            /*
            * calender can change only Period,
            *  So date.firstSelectOptions[1] is used only.
            */
            switch (date.secondSelect) {
          ...
          init: function(successCallback, failCallback) {
            var setDefaultData = function() {
              ...
              scope.pcConditionsDateForm.calenderOptions.minDate = 
                new Date(fullYear - 1, month, date - 1);
              scope.pcConditionsDateForm.calenderOptions.maxDate = 
                new Date(fullYear, month, date);

              minDate = new Date(fullYear - 1, month, date - 1);
              maxDate = new Date(fullYear, month, date);

              scope.pcConditionsDateForm.fromCalenderInput = 
                changeDateFormat(scope.pcConditionsDateForm.fromCalender, true, true);
              scope.pcConditionsDateForm.toCalenderInput = 
                changeDateFormat(scope.pcConditionsDateForm.toCalender, true, true);
          ...
          validate: function() {
            ...
            var MIN_YEAR_FOR_CAMERA = 2000;
            var MAX_YEAR_FOR_CAMERA = 2037;

            if (
              fromDate.getFullYear() < MIN_YEAR_FOR_CAMERA ||
              toDate.getFullYear() > MAX_YEAR_FOR_CAMERA
              ) {
            ...
            if (
              returnVal === true &&
              scope.pcConditionsDateForm.firstSelect === 
                scope.pcConditionsDateForm.firstSelectOptions[1].value
              ) {
              var oneMonthAge = angular.copy(scope.pcConditionsDateForm.toCalender);
              oneMonthAge.setDate(oneMonthAge.getDate() - 31);

              if (
                scope.pcConditionsDateForm.fromCalender.getTime() > 
                  scope.pcConditionsDateForm.toCalender.getTime()
                ) {
                alertMessageLanguageKey = lang.modal.limitStartDay;
                returnVal = false;
              } else if (
                scope.pcConditionsDateForm.secondSelect === 
                  scope.pcConditionsDateForm.secondSelectOptions[0].value &&
                scope.pcConditionsDateForm.fromCalender.getTime() < 
                  oneMonthAge.getTime()
              ) {
              ...
              if (calenderType === 'first') {
                if (
                  ...
                  if (
                    ...
                    date.toCalenderInput = 
                      changeDateFormat(date.calenderOptions.maxDate, true, false);
                  }
                } else {
                  resetMaxDate();
                }
              ...
              if (calenderType === 'second') {
                if (
                  ...
                  if (
                    ...
                    date.fromCalenderInput = 
                      changeDateFormat(date.calenderOptions.minDate, true, false);
                  }
                } else {
                  resetMinDate();
                }
        ...
        function changeDateFormat(date, _useMonth, _useDay) {
          ...
          month = month < OVER_TEN ? "0" + month : month;
          day = day < OVER_TEN ? "0" + day : day;
