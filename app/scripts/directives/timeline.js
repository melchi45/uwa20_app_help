kindFramework.
  directive('timeline', ['$filter', '$interval', '$timeout', '$rootScope',
        ...
        controller: function($scope, $element) {
          ...
          var updateTimelineText = function(list) {
            ...
            if (eventList === null || eventList.length > 1 || eventList[0] === 'All') {
              for (idx = 0; idx < list.length; idx++) {
                list[idx].eventType = playbackInterfaceService.getEventName(list[idx].eventType);
              }
            } else {
              for (idx = 0; idx < list.length; idx++) {
                list[idx].eventType = '';
          ...
          */
          $scope.timelineControl.changeTimelineMode = function(index) {
            ...
            if (index === timelineMode.indexOf('backup')) {
              $scope.visibility.backup = true;
            } else if (index === timelineMode.indexOf('datepicker')) {
        ...
        link: function(scope) {
          ...
          var openOverlapEventPopup = function(eventList) {
            ...
            */
            ...
            scope.control.changeTimelineMode(timelineMode.indexOf('eventsorting'));
          };
