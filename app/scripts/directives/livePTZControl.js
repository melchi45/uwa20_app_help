      ...
      link: function(scope, element, attrs) {
          ...
          apply: function(value) {
            if (scope.addPresetting.SelectedName === null || scope.addPresetting.SelectedName === undefined || scope.addPresetting.SelectedName === "") {
              ModalManagerService.open('message', {
                'buttonCount': 1,
                'message': $translate.instant('lang_msg_validPresetName')
              });
            } else {
              scope.ptzPreset("Add");
              this.show = false;
        ...
        var groupListCallback = function(result) {
          if (result.PTZGroups === undefined) {
            ...
            var groupStartFunc = function(value) {
              run('group', value, 'Start');
            };
            var groupStopFunc = function(value) {
              run('group', value, 'Stop');
            };
