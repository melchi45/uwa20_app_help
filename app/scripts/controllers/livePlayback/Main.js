...
.controller('UniversalMainCtrl', 
  ['$scope', '$location', '$window', '$rootScope','CAMERA_TYPE', 
  ...
  SessionOfUserManager, CAMERA_STATUS, $timeout, $state, RESTCLIENT_CONFIG,$controller) {
    ...
    angular.extend(this, $controller('BaseUniversalMainCtrl', {
      $scope:$scope, $location:$location, $window:$window, $rootScope:$rootScope, 
      $timeout:$timeout, $state:$state, ModalManagerService:ModalManagerService, SunapiClient:SunapiClient, 
      DisplayService:DisplayService, UniversialManagerService:UniversialManagerService, 
      CAMERA_STATUS:CAMERA_STATUS, RESTCLIENT_CONFIG:RESTCLIENT_CONFIG,
    }));
 }]);