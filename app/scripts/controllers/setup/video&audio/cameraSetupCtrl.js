kindFramework.controller('cameraSetupCtrl', function ($scope, $uibModal, $uibModalStack, SunapiClient, Attributes, COMMONUtils, LogManager, CameraSpec, $timeout, $location, MultiLanguage, $rootScope, $q) {
    ...
    $scope.iSSuportedPresetDwellTimeDuration = function () {
        ...
        if($scope.PresetImageConfig[$scope.presetTypeData.PresetIndex].Camera.DayNightMode != 'Auto' || $scope.PresetImageConfig[$scope.presetTypeData.PresetIndex].Camera.AGCMode == 'Off' ||
            (mAttr.PTZModel && ($scope.IRled.Mode == 'On' || $scope.IRled.Mode == 'Sensor' || $scope.IRled.Mode == 'Schedule'))){
            retVal = false;
        }
        return retVal;
    };
    ...
    $scope.getDayNightModeTranslatedOption = function (option) {
        if (mAttr.PTZModel && ($scope.IRled.Mode == 'On' || $scope.IRled.Mode == 'Sensor' || $scope.IRled.Mode == 'Schedule')) {
            return '---';
        } else {
            return COMMONUtils.getTranslatedOption(option)
        }
   ...
   // $scope.previewMode = true;
    ...
    function viewSSDR() {
        return SunapiClient.get('/stw-cgi/image.cgi?msubmenu=ssdr&action=view', '',
                function (response) {
    ...
    function viewImagePreset() {
        return SunapiClient.get('/stw-cgi/image.cgi?msubmenu=imagepreset&action=view', '',
                function (response) {
    ...
    function whitebalanceView() {
        return SunapiClient.get('/stw-cgi/image.cgi?msubmenu=whitebalance&action=view', '',
                function (response) {
    ...
    function imageOptionsView() {
        return SunapiClient.get('/stw-cgi/image.cgi?msubmenu=imageoptions&action=view', '',
                function (response) {
    ...
    function focusView() {
        return SunapiClient.get('/stw-cgi/image.cgi?msubmenu=focus&action=view', '',
                function (response) {
    ...
    function irLEDView() {
        return SunapiClient.get('/stw-cgi/image.cgi?msubmenu=irled&action=view', '',
                function (response) {
    ...
    function setMultiLineOSD(functionList) {
        ...
        if (typeof $scope.DateOSD !== 'undefined' && typeof pageData.DateOSD !== 'undefined') {
            if (!angular.equals(pageData.DateOSD, $scope.DateOSD) || pageData.DateOSD.Enable !== $scope.DateOSD.Enable) {
                promise = function(){ return updateMultiLineOSD($scope.DateOSD[0].Index, 'Date', false, $scope.DateOSD[0]); };
        ...
        if (typeof $scope.TitleOSD !== 'undefined' && typeof pageData.TitleOSD !== 'undefined') {
            if (pageData.TitleOSD.length) {
                if ($scope.TitleOSD.length && pageData.TitleOSD.Enable !== $scope.TitleOSD.Enable) {
                    promise = function(){ return updateMultiLineOSD($scope.TitleOSD[0].Index, 'Title', false, $scope.TitleOSD[0]);};
                    //promises.push(promise);
                    functionList.push(promise);
                }
            } else {
                if ($scope.TitleOSD.length && pageData.TitleOSD.Enable !== $scope.TitleOSD.Enable) {
                    promise = function(){ return addMultiLineOSD($scope.TitleOSD[0].Index, 'Title', false, $scope.TitleOSD[0]);};
                    //promises.push(promise);
                    functionList.push(promise);
                }
            }
            ...
            if (!angular.equals(pageData.TitleOSD, $scope.TitleOSD)) {
                ...
                $scope.TitleOSD.forEach(function(elem, index){
                    if (!angular.equals(pageData.TitleOSD[index], elem)) {
                        if (existedTitleIndex[elem.Index - 1]) {
                            promise = function(){ return updateMultiLineOSD(elem.Index, 'Title', false, elem); };
                            //promises.push(promise);
                            functionList.push(promise);
                        } else {
                            promise = function(){ return addMultiLineOSD(elem.Index, 'Title', false, elem); };
                            //promises.push(promise);
                            functionList.push(promise);
                        }
                    }
                ...
                deletedTitleIndex.forEach(function(elem, index){
                    if (elem === true) {
                        promise = function(){ return removeMultiLineOSD(index + 1, false); };
                        //promises.push(promise);
                        functionList.push(promise);
                    }
                });
