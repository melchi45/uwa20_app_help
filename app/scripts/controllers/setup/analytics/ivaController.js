kindFramework.controller('ivaCtrl', function($scope, $uibModal, $translate, $timeout, SunapiClient, Attributes, COMMONUtils, sketchbookService, $rootScope, $q, eventRuleService, $location, UniversialManagerService) {
    ...
    $scope.onPresetChange = function (Preset)
    {
        ...
        getPresetVideoAnalysis().then(function() {
            ...
            // if($scope.IVAPresetReady !== undefined && $scope.IVAPresetReady === true) {
                ...
                $timeout(function(){updateMDVARegion2($scope.activeTab);},1000);
    ...
    function setPresetVideoAnalysis()
    {
        ...
        for (var k = 0; k < $scope.VA.length - 1; k++) {
            var index = k + 1;
            var setData = {};
            var removeData = {};
    ...
    function setInitialObjectSize() {
        if(typeof $scope.VA[$scope.presetTypeData.SelectedPreset] === 'undefined' || $scope.VA[$scope.presetTypeData.SelectedPreset] === null) {
            return;
        }
    ...
    function setVideoAnalysis() {
        ...
        if ($scope.IntelligentVideoEnable !== false) {
            ...
            if($scope.orgDetectionType[0] === "MDAndIV") {
                url = $scope.va2CommonCmd + '&action=set&DetectionType=MotionDetection';
            } else if($scope.orgDetectionType[0] === "IntelligentVideo" || $scope.orgDetectionType[0] === "Off" || $scope.orgDetectionType[0] === "MotionDetection") {
                url = $scope.va2CommonCmd + '&action=set&DetectionType=Off';
            }
            var getData = {};
    ...
    function openAccordionAll() {
        $('.ui-accordion-header').removeClass('ui-corner-all').addClass('ui-accordion-header-active ui-state-active ui-corner-top').attr({'aria-selected':'true','tabindex':'0'}).find(".ui-icon").removeClass('ui-icon-triangle-1-e').addClass('ui-icon-triangle-1-s');
    ...
    function setCommonMenuDisabled() {
        ...
        $("#iva-setup-tab-sensitivity .slider-fields").css("pointer-events", "none").addClass("cm-opacity");
    }

    function setCommonMenuAbled() {
        $("#iva-setup-tab-size button").attr('disabled', false);
        $("#iva-setup-tab-sensitivity .slider-fields").css("pointer-events", "auto").removeClass("cm-opacity");
    }
