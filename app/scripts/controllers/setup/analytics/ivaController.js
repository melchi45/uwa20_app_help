kindFramework.controller('ivaCtrl', function($scope, $uibModal, $translate, $timeout, SunapiClient, Attributes, COMMONUtils, sketchbookService, $rootScope, $q, eventRuleService, $location, UniversialManagerService) {
  ...
  var UPDATE_UI_TIMEOUT = 1000;
  var NEW_PRESET_SET_TIMEOUT = 500;
  var MIN_OBJECT_HEIGHT_RATIO = 0.022;
  var GET_ATTRIBUTE_TIMEOUT = 500;

  function getCommonCmd() {
    if (typeof $scope.VideoAnalysis2Support !== 'undefined' && 
  ...
  function setVirtualLineMode(index) {
    ...
    if (line.Coordinates.length > 0) {
      ...
      if (line.Mode.indexOf(LINE_MODE.OFF) > -1) {
        ...
        $scope.virtualLineMode.passing = false;
      } else if (line.Mode.indexOf(LINE_MODE.RIGHT) > -1 && 
       (line.Mode.indexOf(LINE_MODE.LEFT) === -1)) {
        $scope.virtualLineMode.right = true;
        $scope.virtualLineMode.left = false;
        $scope.virtualLineMode.both = false;
        $scope.virtualLineMode.currentDirection = 'right';
      } else if (line.Mode.indexOf(LINE_MODE.LEFT) > -1 && 
       (line.Mode.indexOf(LINE_MODE.RIGHT) === -1)) {
        $scope.virtualLineMode.right = false;
  ...
  $scope.selectColumn = function(target) {
    ...
    if (
            typeof $scope.currentTableData[index] === 'undefined' ||
            $scope.currentTableData[index].isEnabled === null ||
            typeof $scope.currentTableData[index].isEnabled === 'undefined'
            ) {
      return;
    }
  ...
  $scope.IncreaseSlider = function (key) {
    if ($scope.sliderOptions.disabled) {
      return;
    }
  ...
  $scope.sizeCoordinateChange = function (tFlag) {
    var flag = parseInt(tFlag);
    var maxWidth = null;
    var maxHeight = null;
    var minWidth = null;
    var minHeight = null;
  ...
  function presetAdd() {
    var queue = [];
    var setData = {};
    var Preset = $scope.PresetNameValueOptions[$scope.presetTypeData.SelectedPreset - 1].split(' : ');
    setData.Preset = Preset[0];
    setData.Name = Preset[1];

    queue.push({
      url: '/stw-cgi/ptzconfig.cgi?msubmenu=preset&action=add',
      reqData: setData
    });
  ...
  function resetSensitivitySlider() {
        // reset sensitivity slider since slider is commonly used all the type.
    if ($scope.VA[$scope.presetTypeData.SelectedPreset].SensitivityLevel <= 0) {
      $scope.VA[$scope.presetTypeData.SelectedPreset].SensitivityLevel = 80;
            // if($scope.presetTypeData.SelectedPreset !== 0) {
      $scope.ivaDurationSliderModel = {
        data : $scope.VA[$scope.presetTypeData.SelectedPreset].SensitivityLevel
      };
            // }
    } else {
            // if($scope.presetTypeData.SelectedPreset !== 0) {
  ...
  $scope.onPresetChange = function (Preset) {
    ...
    $scope.sketchinfo = {};
    getPresetVideoAnalysis().then(function() {
  ...
  $scope.onPresetTypeChange = function (Type) {
    if (Type === 'Global') {
      ...
      if ($scope.orgDetectionType[$scope.presetTypeData.SelectedPreset] === "MotionDetection" || 
       $scope.orgDetectionType[$scope.presetTypeData.SelectedPreset] === "Off") {
        $scope.IntelligentVideoEnable = false;
      } else if ($scope.orgDetectionType[$scope.presetTypeData.SelectedPreset] === "MDAndIV" || 
       $scope.orgDetectionType[$scope.presetTypeData.SelectedPreset] === "IntelligentVideo") {
        $scope.IntelligentVideoEnable = true;
      }
      setInitialObjectSize();
      updateMDVARegion2($scope.activeTab);
      ...
      getPresetVideoAnalysis().then(function() {
        ...
        if (typeof $scope.IVAPresetReady !== 'undefined' && $scope.IVAPresetReady === true) {
                    /*if($scope.orgDetectionType[$scope.presetTypeData.SelectedPreset] === "MotionDetection" || $scope.orgDetectionType[$scope.presetTypeData.SelectedPreset] === "Off") {
                        $scope.IntelligentVideoEnable = false;
                    } else if($scope.orgDetectionType[$scope.presetTypeData.SelectedPreset] === "MDAndIV" || $scope.orgDetectionType[$scope.presetTypeData.SelectedPreset] === "IntelligentVideo"){
                        $scope.IntelligentVideoEnable = true;
  ...
  function gotoPreset(mode, Preset, presetChanged, oldPreset) {
    function oldPresetStop() {
      ...
      return SunapiClient.get('/stw-cgi/ptzconfig.cgi?msubmenu=presetimageconfig&action=set', 
        stopData,
        function (response) {
          $timeout(function() {
            newPreset();
          }, NEW_PRESET_SET_TIMEOUT);
        },
        function (errorData) {
        }, '', true);
    }
  ...
  function getPresetVideoAnalysis(detectionType) {
    ...
    return SunapiClient.get('/stw-cgi/ptzconfig.cgi?msubmenu=presetvideoanalysis2&action=view', 
     getData,
     function (response) {
      // $timeout(function(){
       ...
       for (var i = 0; i < presetVAArray.length; i++) {
         ...
         if (index === $scope.presetTypeData.SelectedPreset) { // when enable of current preset changed, in order to update enable with new data, reset enable UI
           if ($scope.orgDetectionType[index] === "MotionDetection" || 
            $scope.orgDetectionType[index] === "Off") {
             $scope.IntelligentVideoEnable = false;
           } else if ($scope.orgDetectionType[index] === "MDAndIV" || 
            $scope.orgDetectionType[index] === "IntelligentVideo") {
             $scope.IntelligentVideoEnable = true;
           }
         }
         ...
         str = presetVA.ObjectSizeByDetectionTypes[1].MaximumObjectSizeInPixels.split(',');
  ...
  function setPresetVideoAnalysis()    {
    function getRemovedIndex(isArea) {
      ...
      for (var i = 0; i < pageData.VA[$scope.presetTypeData.SelectedPreset][obj].length; i++) {
        self = pageData.VA[$scope.presetTypeData.SelectedPreset][obj][i];
        removeIndexs.push(self[indexKey]);
      }

      for (var j = 0; j < $scope.VA[$scope.presetTypeData.SelectedPreset][obj].length; j++) {
        self = $scope.VA[$scope.presetTypeData.SelectedPreset][obj][j];
    ...
    for (var kk = 0; kk < $scope.VA.length - 1; kk++) {
      var index = kk + 1;
      var setData = {};
      var removeData = {};
      ...
      if ($scope.IntelligentVideoEnable !== false) {
        ...
        if (pageData.VA[index].DisplayRules !== $scope.VA[index].DisplayRules) {
          if ($scope.VA[index].DetectionType === "IntelligentVideo" || 
           $scope.VA[index].DetectionType === "MDAndIV") {
        ...
        if ((pageData.VA[index].MinWidth !== $scope.VA[index].MinWidth) || 
         (pageData.VA[index].MinHeight !== $scope.VA[index].MinHeight)) {
          setData['DetectionType.IntelligentVideo.MinimumObjectSizeInPixels'] = $scope.VA[index].MinWidth + ',' + $scope.VA[index].MinHeight;
          isSetted++;
        }
        if ((pageData.VA[index].MaxWidth !== $scope.VA[index].MaxWidth) || 
         (pageData.VA[index].MaxHeight !== $scope.VA[index].MaxHeight)) {
        ...
        if (!angular.equals(pageData.VA[index].Lines, $scope.VA[index].Lines)) {
          if ($scope.VA[index].Lines.length) {
            for (i = 0; i < $scope.VA[index].Lines.length; i++) {
              coor = [];

              for (j = 0; j < $scope.VA[index].Lines[i].Coordinates.length; j++) {
                coor.push([
              ...
              var VALines = coor.join(',');
              var lineIndex = $scope.VA[index].Lines[i].Line;
              setData["Line." + lineIndex + ".Coordinate"] = VALines;
              ...
              removeIndex = getRemovedIndex(false);
            ...
            if (typeof pageData.VA[index].Lines !== 'undefined') {
        ...
        if (!angular.equals(pageData.VA[index].DefinedAreas, $scope.VA[index].DefinedAreas)) {
          if ($scope.VA[index].DefinedAreas.length) {
            for (i = 0; i < $scope.VA[index].DefinedAreas.length; i++) {
              coor = [];
              if (typeof $scope.VA[index].DefinedAreas[i] !== 'undefined') { 
                for (j = 0; j < $scope.VA[index].DefinedAreas[i].Coordinates.length; j++) {
                  coor.push([
                ...
                if (definedAreaIndex < 9) {
                  ...
                  isSetted++;
                        ...
                        //removeData 찾기
            removeIndex = getRemovedIndex(true);
            ...
            if (typeof pageData.VA[index].DefinedAreas !== 'undefined') {
        ...
        if (isRemoved) {
          removeData.Preset = $scope.VA[index].Preset;
  ...
  $scope.getDisableStatus = function(type) {
    if (typeof $scope.VA[$scope.presetTypeData.SelectedPreset] !== 'undefined' && 
     $scope.VA[$scope.presetTypeData.SelectedPreset].DetectionType === "Off") {
      return true;
    } else if (type === "Analytics" && typeof $scope.VA[$scope.presetTypeData.SelectedPreset] !== 'undefined' && 
     $scope.VA[$scope.presetTypeData.SelectedPreset].DetectionType === "MotionDetection") {
      return true;
    }
  ...
  function getAttributes() {
    ...
    if (typeof mAttr.MotionDetectSensitivityTypes !== 'undefined') {
    ...
    if (typeof mAttr.ROIType !== 'undefined') {
      $scope.ROIType = mAttr.ROIType;
    }
    if (typeof mAttr.VideoAnalyticTypes !== 'undefined') {
    ...
    $scope.$watch('aSliderModel.data', function(newValue, oldValue) {
      if (newValue && newValue !== oldValue) {
        if (typeof $scope.virtualAreaMode !== 'undefined') {
  ...
  function getSelectedPreset() {
    if (typeof mAttr.DefaultPresetNumber === 'undefined' || !mAttr.DefaultPresetNumber) {
      ...
      if (!isCheck) {
        ...
        $scope.onPresetChange($scope.presetTypeData.PresetIndex);
      }
  ...
  function getVideoAnalysis(detectionType) {
    ...
    return SunapiClient.get($scope.va2CommonCmd + '&action=view', getData, function(response) {
      ...
      if (detectionType) {
        ...
        if ($scope.presetTypeData.SelectedPreset === 0) {
          if ($scope.orgDetectionType[0] === "MotionDetection" || 
           $scope.orgDetectionType[0] === "Off") {
                        // $scope.VA[0].DetectionType = "Off"
            $scope.IntelligentVideoEnable = false;
          } else if ($scope.orgDetectionType[0] === "MDAndIV" || 
           $scope.orgDetectionType[0] === "IntelligentVideo") {
      ...
      str = response.data.VideoAnalysis[0].ObjectSizeByDetectionTypes[1].MaximumObjectSizeInPixels.split(',');
  ...
  function setInitialObjectSize() {
    if (typeof $scope.VA[$scope.presetTypeData.SelectedPreset] === 'undefined' || 
     $scope.VA[$scope.presetTypeData.SelectedPreset] === null) {
      return;
    }
    ...
    $scope.minHeightLimit = Math.round(maxHeight * MIN_OBJECT_HEIGHT_RATIO);
  ...
  function setVideoAnalysis() {
    ...
    function getRemovedIndex(isArea) {
      ...
      for (i = 0; i < pageData.VA[0][obj].length; i++) {
        self = pageData.VA[0][obj][i];
        removeIndexs.push(self[indexKey]);
      }

      for (i = 0; i < $scope.VA[0][obj].length; i++) {
        self = $scope.VA[0][obj][i];
        var pageDataLineIndex = removeIndexs.indexOf(self[indexKey]);
        if (pageDataLineIndex > -1) {
          removeIndexs.splice(pageDataLineIndex, 1);
        }
    ...
    if ($scope.IntelligentVideoEnable !== false) {
      ...
      if (pageData.VA[0].DisplayRules !== $scope.VA[0].DisplayRules) {
        if ($scope.VA[0].DetectionType === "IntelligentVideo" || 
         $scope.VA[0].DetectionType === "MDAndIV") {
      ...
      if (!angular.equals(pageData.VA[0].Lines, $scope.VA[0].Lines)) {
        if ($scope.VA[0].Lines.length) {
          ...
          if (typeof pageData.VA[0].Lines !== 'undefined') {
            removeData.LineIndex = 'All';
            isRemoved++;
          }
        }
      ...
      if (!angular.equals(pageData.VA[0].DefinedAreas, $scope.VA[0].DefinedAreas)) {
        if ($scope.VA[0].DefinedAreas.length) {
          for (i = 0; i < $scope.VA[0].DefinedAreas.length; i++) {
            coor = [];

            for (j = 0; j <  $scope.VA[0].DefinedAreas[i].Coordinates.length; j++) {
              coor.push([
          ...
          if (typeof pageData.VA[0].DefinedAreas !== 'undefined') {
            removeData.DefinedAreaIndex = 'All';
            isRemoved++;
          }
        }
      ...
      if ($scope.orgDetectionType[0] === "MDAndIV") {
        url = $scope.va2CommonCmd + '&action=set&DetectionType=MotionDetection';
      } else if ($scope.orgDetectionType[0] === "IntelligentVideo" ||
        $scope.orgDetectionType[0] === "Off" ||
        $scope.orgDetectionType[0] === "MotionDetection") {
        url = $scope.va2CommonCmd + '&action=set&DetectionType=Off';
      }
      var getData = {};
  ...
  function getCurrentDetectionType() {
    if (typeof pageData.VA[$scope.presetTypeData.SelectedPreset] === 'undefined' || 
     pageData.VA[$scope.presetTypeData.SelectedPreset] === null) {
      return null;
    }
    ...
    // function getCurrentPresetDetectionType(){
    //     if(typeof pageData.VA[$scope.presetTypeData.SelectedPreset] === 'undefined' || pageData.VA[$scope.presetTypeData.SelectedPreset] === null) {
    //         return null;
    //     }
    //     var detectionType = null;
    //     var pageDetectionType = pageData.VA[$scope.presetTypeData.SelectedPreset].DetectionType;
      ...
      if ($scope.IntelligentVideoEnable === true) {
        ...
        if ($scope.presetTypeData.SelectedPreset === 0) { // global type
          if ($scope.orgDetectionType[$scope.presetTypeData.SelectedPreset] === "MDAndIV") {
            url = $scope.va2CommonCmd + '&action=set&DetectionType=MotionDetection';
          } else if ($scope.orgDetectionType[$scope.presetTypeData.SelectedPreset] === "IntelligentVideo" || 
           $scope.orgDetectionType[$scope.presetTypeData.SelectedPreset] === "Off") {
            url = $scope.va2CommonCmd + '&action=set&DetectionType=Off';
          }
        } else { // preset type
  ...
  function set(isEnabledChanged) {
    ...
    if (validatePage()) {
      if (!angular.equals(pageData.VA, $scope.VA) ||
        ...
        (detectionType !== $scope.orgDetectionType[$scope.presetTypeData.SelectedPreset])
        ) {
  ...
  $rootScope.$saveOn('<app/scripts/directives>::<updateCoordinates>', function(obj, args) {
    var index = 0, type = 1, points = 2, direction = 3;
    var modifiedIndex = args[index];
    var modifiedType = args[type]; //생성: create, 삭제: delete
    var modifiedPoints = args[points];
    var modifiedDirection = args[direction];
    ...
    if (typeof modifiedPoints !== 'undefined') {
    ...
    switch ($scope.activeTab.title) {
      ...
      case "Virtual Area":
      case "Excluded Area":
        var excludedAreaIndexOffset = 9;
        var includedAredIndexOffset = 1;
        var definedArea = $scope.activeTab.title === "Virtual Area" ? 
            modifiedIndex + includedAredIndexOffset : 
            modifiedIndex + excludedAreaIndexOffset;
        ...
        switch (modifiedType) {
          ...
          case "mouseup":
            ...
            if (modifiedIndex === 0) { // min
              ...
              if (width > $scope.VA[$scope.presetTypeData.SelectedPreset].MaxWidth) {
                width = $scope.VA[$scope.presetTypeData.SelectedPreset].MaxWidth;
              }
              if (height > $scope.VA[$scope.presetTypeData.SelectedPreset].MaxHeight) {
                height = $scope.VA[$scope.presetTypeData.SelectedPreset].MaxHeight;
              }

              $scope.VA[$scope.presetTypeData.SelectedPreset].MinWidth = width;
              $scope.VA[$scope.presetTypeData.SelectedPreset].MinHeight = height;
              $scope.prevMinWidth = width;
              $scope.prevMinHeight = height;
              convertSizeCoordinates(coordinates, 'min');
            } else if (modifiedIndex === 1) { // max
              width = coordinates[2].x - coordinates[0].x;
              height = coordinates[2].y - coordinates[0].y;
              if (width < $scope.VA[$scope.presetTypeData.SelectedPreset].MinWidth) {
                width = $scope.VA[$scope.presetTypeData.SelectedPreset].MinWidth;
              }
              if (height < $scope.VA[$scope.presetTypeData.SelectedPreset].MinHeight) {
                height = $scope.VA[$scope.presetTypeData.SelectedPreset].MinHeight;
              }

              $scope.VA[$scope.presetTypeData.SelectedPreset].MaxWidth = width;
              $scope.VA[$scope.presetTypeData.SelectedPreset].MaxHeight = height;
              $scope.prevMaxWidth = width;
              $scope.prevMaxHeight = height;
  ...
  function findLinesIndex(lineIndex) {
    ...
    for (var ii = 0; ii < $scope.VA[$scope.presetTypeData.SelectedPreset].Lines.length; ii++) {
      var self = $scope.VA[$scope.presetTypeData.SelectedPreset].Lines[ii];
  ...
  $scope.$watch(function() {
    if (typeof $scope.VA !== 'undefined' && 
      typeof $scope.presetTypeData.SelectedPreset !== 'undefined') {
      if (typeof $scope.VA[$scope.presetTypeData.SelectedPreset] !== "undefined") { 
        return $scope.VA[$scope.presetTypeData.SelectedPreset].DetectionType; 
      }
    }
  }, function(newVal, oldVal) {
    if (newVal !== oldVal) {
  ...
  $scope.$watch('analyticsType', function(newVal, oldVal) {
    ...
    var i = 0;
    if (newVal === "Entering") {
      for (i = 0; i < $scope.analyticEnableOptions.length; i++) {
    ...
    if (newVal === "Passing") {
      drawVaLine();
    } else if (newVal === "Entering" || 
     newVal === "Exiting" || 
     newVal === "AppearingDisappearing" || 
     newVal === "Loitering" || 
     newVal === "Intrusion") {
      drawVaArea('Include');
    }
  ...
  function updateMDVARegion2(tabVal, changeOnlyTableData) { // According to tabVal, set $scope.tableData
    ...
    if (typeof $scope.VA[$scope.presetTypeData.SelectedPreset] === 'undefined' || 
     ...
     detectionType === "MotionDetection") {
      ...
      result = [];
            // var startIndex = 0;
      va = $scope.VA[$scope.presetTypeData.SelectedPreset];
      definedAreas = va.DefinedAreas;
      lines = va.Lines;

      if ($scope.activeTab.title === "Virtual Area") {
        if (typeof $scope.VA.SelectedAnalysis === 'undefined') {
          $scope.VA.SelectedAnalysis = $scope.VideoAnalyticTypes[0];
        }
        ...
        updateTableData(areaCount, true);
...
//   };
  ...
  function setPointsToDraw(data, flag) { // set points to $scope.coordinates for sketchInfo
    ...
    var points = [];
    var i = 0;
    var both = 2, right = 1, left = 0;
    if (flag === 'line') { // line
      var line = data;
      var direction = null;
      for (i = 0; i < line.Coordinates.length; i++) {
      ...
      var area = data;
      for (i = 0; i < area.Coordinates.length; i++) {
      ...
      var minSize = data;
      for (i = 0; i < minSize.Coordinates.length; i++) {
      ...
      var maxSize = data;
      for (var kk = 0; kk < maxSize.Coordinates.length; kk++) {
        points.push([maxSize.Coordinates[kk][0], maxSize.Coordinates[kk][1]]);
      }
