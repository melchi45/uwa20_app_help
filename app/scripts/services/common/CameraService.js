...
.service('CameraService', ['$rootScope', 'BaseCameraService','ModalManagerService', 
	...
	cameraService.alarmOut = function() {
		var successCallback = function(response)
		{
			...
			{
				ModalManagerService.open('message', { 'buttonCount': 0, 'message': "lang_alarmoulang_nolang_supported" } );
				return;
			}

			for(var i=0; i<Object.keys(response.data.AlarmOutput).length; i++)
			{
				var Data = {'name': 'Alarm Output ' + (i+1), 'enable': response.data.AlarmOutput[i+1]};
				DataList.push(Data);
			}

			ModalManagerService.open( 'checkboxlist', {'list' : DataList} );
		};

		var errorCallBack = function()
		{
			ModalManagerService.open('message', { 'buttonCount': 0, 'message': "카메라와 연결이 실패했습니다." } );
		};
   	...
   	var successCallback = function(response){
  		...
  		for(var i=0; i<Profilelist.length; i++)
  		{
  			if(Profilelist[i].Name === currentProfileInfo.Name)
  			{
  				...
  				{
  					ModalManagerService.open('message', { 'buttonCount': 0, 'message': "lang_dptzprofile_select" } );
    				return;		
  				}
  				break;
  			}
    	...
    	if(UniversialManagerService.getDigitalPTZ() === CAMERA_STATUS.DPTZ_MODE.DIGITAL_PTZ)
  		{
  			SunapiClient.get(
  				...
  				function() { ModalManagerService.open('message', { 'buttonCount': 0, 'message': "lang_dalang_on" } );	},
  			 	errorCallback,
  			 	'',
  			 	true
  			);
  			...
  			SunapiClient.get(
  				...
  				{},
  				function() { ModalManagerService.open('message', { 'buttonCount': 0, 'message': "lang_dalang_off" } );	},
  				errorCallback,
  				'',
  				true
  			);
		...
		var errorCallback = function(){
    	ModalManagerService.open('message', { 'buttonCount': 0, 'message': "lang_camera_conneclang_failed" } );	
    };
