      ...
      var ItemSetModel = function() {
        ...
        */
        ...
        var makeTimelineItem = function(inputArray) {
          ...
          for( var i=0 ; i< r.length ; i++ ) {
            if( i < WARNING_COUNT ) {
              newArray.push(r[i]);
              markingArray(r[i].start.getTime()/1000, r[i].end.getTime()/1000);
            }
            else {
              var timeInfoList = getTimeInfo(r[i].start, r[i].end);
        ...
        this.addData = function(newArray, showSequence) {
          ...
          if( showSequence === true && itemSet.dupEventData.length !== 0 ) {
            for( var i=0 ; i< itemSet.timelineData.length; i++) {
              if( i+1 < itemSet.timelineData.length &&  itemSet.timelineData[i+1].start - itemSet.timelineData[i].end === 1000 ) {
        ...
        */
        var getAllRangedData = function( targetArray, start, end ) {
          ...
          while( min <= max) {
            ...
            if( targetArray[guess].start > start ) {
              if( guess === 0 || targetArray[guess-1].end < start ) {
                startIndex = guess;
                break;
              }
              max = guess- 1;
              ...
              if( targetArray[guess].end > start ) {
                if( guess !== 0 && 
                      targetArray[guess-1].start.getTime() === targetArray[guess].start.getTime() ) {
                  max = guess -1;
                }
                else {
                  startIndex = guess;
          ...
          while( min <= max ) {
            guess = Math.floor((max+min)/2);
            if( newTargetArray[guess].start >= end ) {
              if( guess !== 0 &&
                  newTargetArray[guess-1].start < end ) {
                endIndex = guess -1;
                break;
              }
              max = guess - 1;
        ...
        */
        var binarySearch = function( targetArray, time ,direction) {
          ...
          while( min <= max ) {
            guess = Math.floor((max+min)/2);
            var results = checkTimePosition(targetArray[guess].start, targetArray[guess].end);
            if( results > 0  ) {
              max = guess-1;
            } 
            else if( results === 0 ) {
          ...
          if( guess < targetArray.length && 
            checkTimePosition(targetArray[guess].start, targetArray[guess].end) === 0 ) {
        ...
        */
        this.getNextNearItem = function(start) {
          ...
          while( min <= max ) {
            guess = Math.floor((max+min)/2);
            if( itemSet.timelineData[guess].start >= start ) {
              if( guess === 0 || itemSet.timelineData[guess-1].end <= start) {
                break;
              }
              max = guess -1;
            } else {
          ...
          if( guess < itemSet.timelineData.length && itemSet.timelineData[guess].start >= start ) {
        ...
        var checkDuplicateEvent = function(objectList){
          ...
          for(var index = 0 ; index < targetList.length; index++ ){
            ...
            while( recalculated === true ) {
              recalculated = false;
              enableDup = false;
              var rangeList = getAllRangedData(newTargetList, startPosition, endPosition);
              if( rangeList === null ) {
                break;
              }
              ...
              */
              for( var i=0 ; i< itemContainning.length; i++ ) {
                if( itemContainning[i].start < startPosition ) {
                  startPosition = itemContainning[i].start;
                  recalculated = true;
                }
                if( itemContainning[i].end > endPosition ) {
                  endPosition = itemContainning[i].end;
                  recalculated = true;
                }
              }
            }
            if( enableDup ) {
              ...
              */ 
              var dupItem = { id :"dup"+dupId, start:startPosition, end:endPosition, 
                              className : DUPLICATE_CLASS_NAME, dupId : dupId,
                              length : endPosition.getTime() - startPosition.getTime() };
