...
kindFramework.directive('liveFunctions', 
  [ 'Attributes', 'CameraService', 'PTZ_TYPE','CAMERA_STATUS', 
    ...
    return {
        ...
        link: function(scope, element, attributes) {
          ...
          var changeZoomMode = function() {
            ...
            {
              case CAMERA_STATUS.ZOOM_MODE.DIGITAL_ZOOM: zoomMode = "Digital Zoom"; break;
              case CAMERA_STATUS.ZOOM_MODE.OPTICAL_PTZ: zoomMode = "PTZ";  break;
              case CAMERA_STATUS.ZOOM_MODE.DIGITZL_PTZ: zoomMode = "Digital PTZ";  break;
            }
            ...
            $timeout(function() {
              scope.$apply(function(){
                ...
                for(var itemIndex=0, slideIndex=0, totalItemCount=0; itemIndex < functionsList.length; slideIndex++)
                {
                  ...
                  for(itemCount =0; (itemCount < functionsList.length) && (itemIndex < functionsList.length) && (itemCount < MAXIMUM_COUNT_ICON_SLIDES); itemIndex++)
                  {            
          ...
          if (result.PTZPresets === undefined) {
            ModalManagerService.open('message', { 'buttonCount': 1, 
                                                  'message': 'lang_NoListFound' });
            } else {          
              var list = [];
              var presetFunc = function(value) { DigitalPTZContorlService.runDPTZ('preset', value); };
              for(var index = 0 ; index < result.PTZPresets[0].Presets.length; index++ ) {
                list[index] = {'name':result.PTZPresets[0].Presets[index].Name,
                                'action': presetFunc,
              ...
              ModalManagerService.open('presetlist', { 'buttonCount' : 1, 'list' : list, 'homeAction' : homeAction, 'addAction' : addAction } );
            }
          };
          ...
          var groupListCallback = function(result) {
            ...
            if(result.PTZGroups === undefined) {
              ...
              for(var i = 1; i <= groups.length; i++) {
                list.push({'name':groupPrefix + ' ' + i,
                           'action': groupFunc,
                           'value':i,
                });
              }
              ModalManagerService.open('liveList', { 'buttonCount' : 0, 'list' : list } );
            }
          };
