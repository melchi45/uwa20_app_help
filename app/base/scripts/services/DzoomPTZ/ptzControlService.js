kindFramework.factory('PTZContorlService', ['$q', 'LoggingService', 'SunapiClient', 'PTZ_TYPE', 'Attributes', '$rootScope',
  ...
  function($q, LoggingService, SunapiClient, PTZ_TYPE, Attributes,
    ...
    function eventHandler(event, eventType, element) {
      ...
      if (eventType === "mousewheel") {
        ...
        if (downCheck) {
          ...
          if (!checkBoundary(event.offsetX, event.offsetY)) {
            return;
          } // Checking End Point
    ...
    function runPTZAreaZoom(X1, Y1, X2, Y2, TileWidth, TileHeight) {
      ...
      turnOffTracking();
      return execSunapi(sunapiURI, function() {
        PTStatus = "MOVING";
        ZStatus = "MOVING";
        setPTZAreaZoom("start");
        blnSavePTZCoordinate = true;
      });
    }
    ...
    function getPTZAreaZoomURI(mode) {
      ...
      if (ptzAreaZoomCurrent > 0 && mode === "prev") {
        ptzAreaZoomStart = true;
        execSunapi(ptzAreaZoomList[--ptzAreaZoomCurrent], function() {
          PTStatus = "MOVING";
          ZStatus = "MOVING";
          setPTZAreaZoom("start");
        });
    ...
    function getMode(type) {
      return (ptzAreaZoomMode === true && type === "Areazoom") ? ptzModeList.AREAZOOM : ptzMode;
    }
    ...
    function getManualTrackingMode() {
      return manualTracking;
    }

    function getAutoTrackingMode() {
      return autoTracking;
    }
