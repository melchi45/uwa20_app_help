kindFramework.factory('SunapiConverter', function($q) {
    ...
    for (var i = 0; i < _lineArr.length; i++) {
      if (_lineArr[i] !== '') {
        ...
        if (findChannelIndex === -1) {
          _obj['VideoProfilePolicies'].push({
            "Channel": _channel
          });
    ...
    for (var i = 0; i < _lineArr.length; i++) {
      if (_lineArr[i] !== '') {
        ...
        if (findProfileIndex === -1) {
          _obj['VideoProfiles'][findChannelIndex]['Profiles'].push({
            "Profile": _profileNo
          });
  ...
  var convertToTimeline = function(res) {
    ...
    for (var i = 1; i < _lineArr.length; i++) {
      ...
      if (findChannelIndex === -1) {
        _obj['TimeLineSearchResults'].push({
          "Channel": _channel,
          "Results": []
        });
      ...
      if (findResultIndex === -1) {
        _obj['TimeLineSearchResults'][findChannelIndex]['Results'].push({
          "Result": _result
        });
