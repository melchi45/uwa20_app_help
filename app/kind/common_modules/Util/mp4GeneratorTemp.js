...
mdhd = function(track) {
  var result = new Uint8Array([
    ...
    0x00, 0x00, 0x27, 0x10, // timescale, 1,0000 "ticks" per second
...
sidx = function(size, ept) {
  var
    bytes = new Uint8Array([
      ...
      //0x00, 0x01, 0x5F, 0x90, // timescale, 90,000 "ticks" per second
	  0x00, 0x00, 0x27, 0x10, // timescale, 10,000 "ticks" per second
      0x90, 0x00, 0x00, 0x00
    ]);
...
mvhd = function(duration) {
  var
    bytes = new Uint8Array([
      ...
      //0x00, 0x01, 0x5F, 0x90, // timescale, 90,000 "ticks" per second
      0x00, 0x00, 0x27, 0x10, // timescale, 1,000 "ticks" per second
	  //0x00, 0x00, 0x03, 0xE8, // timescale, 1,000 "ticks" per second
...
function mediaSegment(sequenceNumber, tracks, data, ept) {
  ...
  result = new Uint8Array(stypBox.byteLength +moofBox.byteLength + frameData.byteLength);
  result.set(stypBox);
  result.set(moofBox, stypBox.byteLength);
  result.set(frameData, stypBox.byteLength + moofBox.byteLength); 
  
  /* result = new Uint8Array(stypBox.byteLength + sidxBox.byteLength +moofBox.byteLength + frameData.byteLength);
  result.set(stypBox);
